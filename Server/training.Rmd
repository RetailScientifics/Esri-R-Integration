...
The purpose of this notebook is to train a toy model to demonstrate how R modeling can be tied in with web-based frontends. In this case, we will use a shapefile that contains latitudes, longitudes, and a number of demography variables for polygons around the US. We'll mock up a variable that represents sales or revenue in each district, and create a model to predict that revenue.

The model we'll be fitting is a simple [linear regression](http://r-statistics.co/Linear-Regression.html), which may or may not have predictive power in this case, but is simple to set up and use.
...
We've downloaded and unzipped it into the 'files' subdirectory, so we can now read it into R using either the rgdal library or Esris' [Arcgis r-bridge package](https://github.com/R-ArcGIS/r-bridge).
```{r}
...
Next, we'll add a "Revenue" column that will serve as our response variable with an average of about $100,000, and look at some summary statistics to get an idea of how it is actually distributed:
...
From this cleaned data set, we'll create some derived variables from the remaining columns, while dropping the columns they originally came from. This allows us to simulate categorical variables, and is also a technique that can be used when the exact numerical value of a regressor isn't especially important, but one still cares about roughly where it falls within the original distribution.
```{r}
dfClean2 <- dfClean %>%
...
So by examining the MAPE for example, we find that our training error is roughly 6.67% while the error on the testing set is about 8.97%.
